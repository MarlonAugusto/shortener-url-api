...
describe('AuthController', () => {
  ...
  jest.mock('src/shortener/models/url.entity', () => ({
    UrlEntity: class {},
  }));
  ...
  describe('register', () => {
    it('deve registrar um usuário com sucesso quando as senhas coincidem', async () => {
      ...
      jest.spyOn(authService, 'createUser').mockResolvedValue({ id: 1, email: 'test@example.com' });
    ...
    it('deve lançar BadRequestException quando ocorre um erro no serviço', async () => {
      ...
      jest.spyOn(authService, 'createUser').mockRejectedValue(new Error('Erro no serviço'));
  ...
  describe('login', () => {
    ...
    it('deve lançar BadRequestException quando ocorre um erro no serviço', async () => {
      ...
      jest.spyOn(authService, 'login').mockRejectedValue(new Error('Erro no serviço'));

      const res = {
        cookie: jest.fn(),
      } as unknown as Response;

      await expect(controller.login(email, password, res)).rejects.toThrow(BadRequestException);
      expect(authService.login).toHaveBeenCalledWith(email, password);
    });
  });
